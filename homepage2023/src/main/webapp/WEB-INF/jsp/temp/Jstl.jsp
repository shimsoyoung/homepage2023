...
<ul>
	...
	<c:when test="${step eq '4'}">
		...
		<c:when test="${step eq '6'}">
			<li>
				<h3>c:forEach Tag : 기본 반복 태그, 고정 된 횟수 또는 초과 수집 동안 충첩 된 본문 내용을 반복</h3>
				<h4>기본문법 : </br>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				URL 1 : <c:url var="link1" value="http://www.naver.com"/> <br/>
				URL 2 : <c:url var="link2" value="/temp/jstlImport.do"> <br/>
							<c:param name="test" value="테스트 호출"/>
						</c:url>
						<br/>
				
				<a href="${link1}" target="_blank">링크1 번</a><br/>
				<a href="${link2}" target="_blank">링크2 번</a>
			</li>
		</c:when>
		...
		<c:when test="${step eq '11'}">
			<li>
				<h3>fn:startsWith() : 주어진 문자열이 특정 문자열 값로 시작하는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '12'}">
			<li>
				<h3>fn:endsWith() : 주어진 문자열이 특정 문자열 값로 시작하는지 확인하는데 사용</h3>
